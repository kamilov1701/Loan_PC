...
export default function Analytics() {
  ...
  if (!stats || stats.error || !stats.status_data) {
    return (
      <div className="p-20 text-center flex flex-col items-center gap-4">
        <span className="text-red-500 font-medium">Error loading analytics data. Please try again. {stats?.error}</span>
      </div>
    );
  }
    ...
    <div className="font-sans pb-20 pt-10 px-4 md:px-8 space-y-16 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <section className="container mx-auto space-y-6">
        <div className="flex flex-wrap gap-6 items-center">
          <div className="flex bg-white dark:bg-slate-900 p-1 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-x-auto">
            {["Today", "Last 3 Days", "Last 7 Days", "Monthly", "Yearly", "All Time"].map(t => (
              <button 
                key={t}
                onClick={() => {
                  setFilter(t);
                ...
                className={`px-6 py-2.5 whitespace-nowrap rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300 ${filter === t ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/30" : "text-slate-500 hover:text-slate-900 dark:hover:text-white"}`}
              >
                {t}
              </button>
            ))}
      ...
      <section className="container mx-auto">
        <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-sm border border-slate-100 dark:border-slate-800 p-10 hover:shadow-lg transition-all">
          <div className="flex items-center justify-between mb-10">
            <div className="flex flex-col gap-1">
              <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase italic">Recent Activities</h3>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Detailed loan distribution</p>
            </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 text-[10px] uppercase tracking-widest text-slate-400">
                  ...
                  <th className="pb-4 font-black">Staff / Casser</th>
                ...
                {stats.recent_logs && stats.recent_logs.length > 0 ? (
                  stats.recent_logs.map((log, index) => (
                    <tr key={index} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="py-4 text-xs">{new Date(log.created_at).toLocaleString()}</td>
                      <td className="py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-500">
                            {(log.casser_name || 'U').charAt(0)}
                          </div>
                          {log.casser_name || 'System'}
                        </div>
                      </td>
                      <td className="py-4">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                          log.action_type === 'added_loaner' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30' :
                          log.action_type === 'reduced_loan' ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30' :
                          'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30'
                        }`}>
                          {log.action_type.replace('_', ' ')}
                        </span>
                      </td>
                      <td className="py-4 text-xs font-semibold">{log.details}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan="4" className="py-8 text-center text-slate-400 text-xs uppercase tracking-widest">
