  ...
  const fetchUserData = async () => {
    try {
      ...
      if (Array.isArray(loanersRes)) {
          ...
          setTodayLoans(loanersRes.filter(l => l.start_date === todayStr || (l.created_at && l.created_at.startsWith(todayStr))));
          setStats({
            ...
            <div className="lg:col-span-8 xl:col-span-9 space-y-8">
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-xl">
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-4 custom-scrollbar">
                        {(user.role === 'Admin' ? cassers.filter(c => String(c.id) === String(user.id)) : cassers).length === 0 ? (
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest p-4">No activity today</p>
                        ) : (
                            (user.role === 'Admin' ? cassers.filter(c => String(c.id) === String(user.id)) : cassers).map(admin => (
                                <div 
                                    key={admin.id} 
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-wider">{admin.today_count || 0} Clients Issued</p>
                                        </div>
                                    </div>
                                    <p className="text-lg font-black text-slate-900 dark:text-white tracking-tighter mb-2">
                                        {parseInt(admin.today_lent_total || 0).toLocaleString()} <span className="text-[10px] opacity-50 font-bold uppercase">UZS</span>
                                    </p>
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase italic tracking-tight flex items-center gap-2">
                                <HiCash className="text-indigo-500" /> Bugungi Savdo
                            </h3>
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Today's Issued Loans</p>
                        </div>
                    </div>
                    <div className="flex gap-6 overflow-x-auto pb-4 custom-scrollbar">
                        {todayLoans.length === 0 ? (
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest p-4">Bugun savdo yo'q</p>
                        ) : (
                            todayLoans.map(loan => (
                                <div key={loan.id} className="min-w-[280px] bg-indigo-50/50 dark:bg-indigo-500/5 p-6 rounded-[32px] border border-indigo-100 dark:border-indigo-500/20 relative overflow-hidden group">
                                    <div className="flex items-center gap-4 mb-4">
                                        <div className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900 flex items-center justify-center text-sm font-black text-indigo-500 shadow-sm overflow-hidden border border-slate-100 dark:border-slate-800">
                                            {loan.image ? <img src={`${import.meta.env.VITE_API_URL}${loan.image}`} className="w-full h-full object-cover" /> : loan.name[0].toUpperCase()}
                                        </div>
                                        <div>
                                            <h4 className="text-[12px] font-black text-slate-900 dark:text-white uppercase tracking-tighter truncate w-32">{loan.name}</h4>
                                            <p className="text-[9px] font-bold text-indigo-500 uppercase tracking-wider">{loan.phone_number}</p>
                                        </div>
                                    </div>
                                    <p className="text-lg font-black text-rose-600 dark:text-rose-400 tracking-tighter mb-2">
                                        {parseInt(String(loan.loan_amount).replace(/[^0-9]/g, '') || 0).toLocaleString()} <span className="text-[10px] opacity-60 font-bold uppercase">UZS</span>
                                    </p>
                                    <div className="pt-3 border-t border-indigo-100 dark:border-indigo-500/20">
                                        <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 flex justify-between items-center">
                                            <span>Deadline:</span>
                                            <span className="text-indigo-600 dark:text-indigo-400">{loan.deadline}</span>
                                        </p>
                                    </div>
                                </div>
                            ))
