...
export default function ListLoan() {
  ...
  const [loanerToDelete, setLoanerToDelete] = useState(null);
  ...
  const handleDeleteClick = (id) => {
    setLoanerToDelete(id);
    setShowDeleteConfirm(true);
  };

  const confirmDelete = async () => {
    if (!loanerToDelete) return;
    try {
      const formData = new FormData();
      formData.append('id', loanerToDelete);
      const res = await fetch(`${import.meta.env.VITE_API_URL}delete_loaner.php`, {
        method: 'POST',
        body: formData
      });
      const data = await res.json();
      if (data.success) {
        setLoaners(loaners.filter(l => l.id !== loanerToDelete));
      }
    } catch (err) {
      console.error(err);
    } finally {
      setShowDeleteConfirm(false);
      setLoanerToDelete(null);
    }
  };
    ...
    <div className="pb-10">
                    ...
                    <div className='flex gap-4 items-center flex-1 px-4 justify-center'>
                      ...
                      {(user.role === 'Programmer' || user.role === 'Owner') && (
                        <>
                          ...
                          <button onClick={() => handleDeleteClick(loaner.id)} className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-slate-400 hover:text-red-600 transition-all"><Trash2 size={16} /></button>
                        </>
                      )}
                    </div>
                  </div>
                    ...
                    <div className='flex flex-wrap justify-between items-center gap-2'>
                      ...
                      {(user.role === 'Programmer' || user.role === 'Owner') && (
                        <>
                          ...
                          <button onClick={() => handleDeleteClick(loaner.id)} className='flex-1 min-w-[45px] py-2.5 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-xl flex items-center justify-center transition-all'><Trash2 size={16} /></button>
                        </>
                      )}
                    </div>
                  </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="relative max-w-sm w-full bg-white dark:bg-slate-900 rounded-[32px] overflow-hidden shadow-2xl border border-white/10 p-8 text-center">
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">Tasdiqlash</h3>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-8">Foydalanuvchini haqiqatdan ham o'chirmoqchimisz?</p>
            <div className="flex gap-4">
              <button 
                onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 px-6 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-black uppercase tracking-widest rounded-2xl transition-all active:scale-95 hover:bg-slate-200 dark:hover:bg-slate-700"
              >
                Yo'q
              </button>
              <button 
                onClick={confirmDelete}
                className="flex-1 px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-rose-500/30 transition-all active:scale-95"
              >
                Ha
              </button>
            </div>
